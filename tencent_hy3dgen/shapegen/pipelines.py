...
def instantiate_from_config(config, **kwargs):
    ...
    try:
        ...
        target = config['target'].replace("hy3dgen", "tencent_hy3dgen")
        cls = get_obj_from_str(target)
    except Exception as e:
        target = config['target'].replace("hy3dshape", "tencent_hy3dgen.shapegen")
        cls = get_obj_from_str(target)
...
class Hunyuan3DDiTPipeline:
    ...
    def set_surface_extractor(self, mc_algo):
        ...
        logger.info('The parameters `mc_algo` is deprecated, and will be removed in future versions.\n'
                    'Please use: \n'
                    'from tencent_hy3dgen.shapegen.models.autoencoders import SurfaceExtractors\n'
